...
Helmsman projects use a concept called iPool (idea pool) which is collection of ideas, wants, desires, and implementations that represents a significant amount of the "purpose" for this hProjets. The iPool may be stored in the hDocs/tempPool file in the repo, but is most often stored in a lucid chart. If the user asks for anything related to the iPool, first check the hDocs/remotes#lucidchart for instructions for accessing the iPool, then check hDocs/tempPool if no other alternative exists. 
...
Document contains a loosley structured jot of the user's wants, questions, and wonders. 
