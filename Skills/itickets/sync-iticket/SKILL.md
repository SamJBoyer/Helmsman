...
Shared file format, filename rules, and matching rules: [ITICKET-FORMAT.md](../ITICKET-FORMAT.md).
...
For each open GitHub issue with the itag, check whether a local file already references it using the [matching rule](../ITICKET-FORMAT.md#matching-rule).

For each open issue with no matching local file:

1. Create a new markdown file in `.scratch/.itickets/<itag>`.
2. Use the [filename convention](../ITICKET-FORMAT.md#filename). Avoid collisions by appending the issue number if needed (e.g. `colors-unappealing-1.md`).
3. Use the [iticket template](../ITICKET-FORMAT.md#template). Set `description:` from the issue body (or title if the body is empty). Set `github-issue:` to the issue URL.
